...
function MapIconFilled({ size = 14, color = "currentColor" }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={color} xmlns="http://www.w3.org/2000/svg">
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-map { display: inline-flex; align-items: center; gap: 5px; background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 6px; padding: 5px 10px; font-size: 11px; font-weight: 700; color: #1d4ed8; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: all 0.15s; }
  .btn-map:hover { background: #1d4ed8; color: #fff; border-color: #1d4ed8; }
      ...
      <div className="layout">
        ...
        <div className="content">
          <header className="topbar">
            <div>
              <div className="topbar-title">Rotas</div>
              <div className="topbar-sub">Rotas cadastradas no sistema</div>
            </div>
          ...
          <main className="main">
            <div className="top-bar">
              <h2 className="page-title">Rotas cadastradas</h2>
              ...
              <button className="btn-primary" onClick={() => router.push("/cadastro_rota")}>
                Cadastrar rota
              </button>
              <button className="btn-primary" onClick={() => router.push("/lista_escolas")}>
                Gerenciar escolas
              </button>
            </div>
                        ...
                        <td>
                          <div className="td-ops">
                            <button className="btn-map" onClick={() => router.push(`/visualizar_rota?id=${r.id}`)}>
                              <MapIconFilled size={13} /> Mapa
                            </button>
                            <button className="btn-edit" onClick={() => router.push(`/editRota?id=${r.id}`)}>
